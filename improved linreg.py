...
class LinReg:
    ...
    def gd_fit(self, X_train, y_train, alpha, threshold):
        ...
        self.theta = self.theta - alpha*xpartial
        ...
        while cost1-cost2 > threshold:
            ...
            if cost1-cost2 > 1:
                self.theta = self.theta - alpha * self.partial(m_samples, self.theta, X_train, y_train)*2
            else:
                self.theta = self.theta - alpha*5   * self.partial(m_samples, self.theta, X_train, y_train)**2
            cost1 = cost2
            cost2 = self.cost(self.theta, m_samples, X_train, y_train)
            epochs += 1
    ...
    def cost(self, array, m_samples, X_train, y_train):
        ...
        for k in range(0,m_samples):
            sum += (np.dot(X_train.iloc[k], array)-y_train.iloc[k])**2
    ...
    def partial(self, m_samples, theta, X_train, y_train):
        ...
        for i in range(0, m_samples):
            vectorsum.append(np.dot(X_train.iloc[i], theta) - y_train.iloc[i])
        s = pd.Series(vectorsum).astype(str).str.zfill(11)
        s = pd.to_numeric(s, errors='coerce').fillna(0, downcast='infer')
        return np.dot(s, X_train)/m_samples
